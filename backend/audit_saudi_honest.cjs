...
async function runSaudiAudit() {
  console.log('🇸🇦 Auditoría Honesta: Saudi Pro League');
  ...
  let sbData;
  try {
    const sbUrl = `https://site.api.espn.com/apis/site/v2/sports/soccer/ksa.1/scoreboard?dates=${DATE_RANGE}&limit=200`;
  ...
  for (const match of matches) {
    ...
    try {
      ...
      const poissonProbs = calcProbs ? calcProbs(hGF, hGA, aGF, aGA, 'Saudi Pro League') : null;
      ...
      picks.forEach(p => {
        ...
        total++;
        const isSurvivalPick = p.argument?.includes('[🚨 Duelo de Miedo]') || p.argument?.includes('[🔥 Survival Boost]');
      ...
      if (matchReport.picks.some(p => p.isSurvivalPick)) {
        reports.push(matchReport); // Solo guardamos para mostrar si tuvieron picks de contexto
      }
  ...
  console.log('\n\n' + '═'.repeat(60));
  console.log('🏆 INFORME AUDITORÍA: SAUDI PRO LEAGUE (ABRIL-MAYO 2026)');
  console.log('═'.repeat(60));
...
runSaudiAudit().catch(console.error);
