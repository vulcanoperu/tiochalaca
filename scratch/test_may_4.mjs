...
async function getMatches(dateStr) {
  const data = await fetchJson(`https://site.api.espn.com/apis/site/v2/sports/soccer/all/scoreboard?dates=${dateStr}`);
  let matches = [];
...
async function run() {
  console.log('Fetching matches for 20260504...');
  const matches = await getMatches('20260504');
  console.log(`Found ${matches.length} completed matches. Filtering top 15...`);
  ...
  for (const m of matches.slice(0, 15)) {
    ...
    if (res && res.picks.length > 0) {
      analyzed++;
      console.log(`\n============================`);
      console.log(`⚽ ${res.match} (Real: ${res.result})`);
      const hg = res.realHG;
      const ag = res.realAG;
      const tg = hg + ag;
      ...
      res.picks.forEach(p => {
        ...
        if (p.tier !== '🔥') { // skip live strategies
          totalPicks++;
          if (won) wonPicks++;
          console.log(`  [${won ? '✅' : '❌'}] ${p.selection} (${p.probability}%) ${p.tier === '💎' ? '💎 VALUE' : ''}`);
        }
      });
    }
  }
  ...
  console.log(`\n🏆 RESULTADOS FINALES`);
  console.log(`Total Picks Evaluados: ${totalPicks}`);
  console.log(`Aciertos: ${wonPicks}`);
  console.log(`Win Rate: ${totalPicks > 0 ? ((wonPicks / totalPicks)*100).toFixed(1) : 0}%`);
}
